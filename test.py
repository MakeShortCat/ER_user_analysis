...
stopwords= ['임', '면', '나', '게', '거', '데','요','!', '합니다', '-','...','.','..', '이', '있', '하', '것', '들', '그', '되', '수', '이', '보',' ', '에', '을', ',', '?', '은',
              ...
              '부탁', '<', '>', '-', '->', '<-', '리턴','이라', '들은', '이터널', '📣', '𐅀', '어어', '터널',
                '해줘','냐', '없이', '많이','라는', '처럼', '계속','놈','끼리','이서','건', '된', '이고', '생각']
...
# word_func = section_ploting_node('2020-12-06' ,'2021-01-10')
# word_func = section_ploting_node('2021-01-10' ,'2021-04-20')
# word_func = section_ploting_node('2021-04-20', '2022-05-22')

# word_func = section_ploting_node('2022-05-22', '2022-06-25')
word_func = section_ploting_node('2022-06-25', '2023-05-16')
...
for word, connections in word_func[0].items():
    for target_word, weight in connections.items():
        if weight >= 10:  # 가중치가 2 이상일 때만 엣지를 추가
...
# 노드와 엣지 그리기
pos = nx.spring_layout(G, k = 2, seed = 17, iterations = 75)

# 노드 그리기 (노드 크기는 단어 등장 횟수에 비례, 색상은 word_colors 딕셔너리에 따름)
nx.draw_networkx_nodes(G, pos, 
                       node_size=[G.nodes[u].get('size', 5)*50 for u in G.nodes()], 
                       node_color=[color_dict.get(u, 'gray') for u in G.nodes()])

# 그 다음, 엣지를 그립니다. 이 때, 엣지의 두께는 가중치에 비례하도록 합니다.
edges = nx.draw_networkx_edges(G, pos, edge_color=[color_dict.get(u[0], 'gray') for u in G.edges()],
                               width=[G[u][v]['weight']/5 for u, v in G.edges()])

# 라벨 그리기
labels = nx.draw_networkx_labels(G, pos, font_size=18, font_weight='bold')
